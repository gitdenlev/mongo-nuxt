<template>
  <div class="relative">
          ...
          <div class="flex flex-col h-full justify-end">
            ...
            <Transition>
              <div v-show="isOpen" class="custom-modal p-2">
                <ul>
                  <li @click="toggleSettings" class="cursor-pointer p-2 hover:bg-gray-600 flex items-center gap-3 rounded-xl">
                    <Icon name="mingcute:settings-3-fill" size="30" />Settings
                  </li>
                  <li @click="handleLogout" class="cursor-pointer p-2 hover:bg-gray-600 flex items-center gap-2 rounded-xl">
    ...
    <!-- Mobile icon -->
    <div @click="isSidePanel = true" class="block md:hidden cursor-pointer fixed top-4 left-4">
    ...
    <Transition name="slide-left">
      <div v-if="!isSidePanel" class="hidden md:flex fixed top-0 left-0 w-20 h-screen p-4 flex-col justify-between items-center bg-gray-900 text-white border-r border-gray-800/30 z-20">
        <div class="flex flex-col items-center gap-4">
          <Icon name="game-icons:mountaintop" size="40" class="text-gray-300" />
          <button class="sidepanel-btn transition-all duration-300 bg-gray-700 p-2 rounded-xl" @click="isSidePanel = true">
            <Icon name="fluent:panel-left-16-regular" size="30" />
          </button>
          <div class="sidepanel-btn transition-all duration-300 bg-gray-700 p-2 rounded-xl">
            <ThemeButton />
          </div>
        </div>
        <button class="sidepanel-btn transition-all duration-300 bg-gray-700 p-2 rounded-xl hover:scale-110">
